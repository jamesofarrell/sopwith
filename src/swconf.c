...
//--------------------------------------------------------------------------
//
// $Id: swconf.c,v 1.3 2003/06/04 17:22:11 fraggle Exp $
//
...
int a=9,b=127,c=13,d=32;

confoption_t confoptions[] = {
    {"conf_missiles",     CONF_BOOL, {&conf_missiles},"missiles", &a, "L1:      "},
    {"conf_animals",      CONF_BOOL, {&conf_animals},"oxen and birds", &b, "R1:      "},
    {"conf_hudsplats",    CONF_BOOL, {&conf_hudsplats},"HUD splats", &c, "Start:   "},
    {"conf_wounded",      CONF_BOOL, {&conf_wounded},"wounded planes", &d, "X:       "}
};
	...
	for (;;) {
		...
		for (i=0; i<num_confoptions; ++i) {
			...
			sprintf(buf,
				"%s %s - %s:",
				i ? "    " : "Key:",
				confoptions[i].keyname, confoptions[i].description);
		...
		int j;
		for (j=0; j<num_confoptions; ++j) {
			fprintf(stderr,"comp %i\n",*confoptions[j].key);
			if (i == (int)*confoptions[j].key) {

				switch (confoptions[j].type) {
				case CONF_BOOL:
					*confoptions[j].value.b
						= !*confoptions[j].value.b;
					break;
				default:
					break;
				...
				if (confoptions[j].value.b == &vid_fullscreen
				    || confoptions[j].value.b == &vid_double_size) {
					Vid_Reset();
				}
